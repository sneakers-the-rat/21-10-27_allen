...
<FlexBox flexDirection={'row'} justifyContent={'flex-start'} marginLeft={"0px"}>
 <Box width = {1/2}>
   ...
   <UnorderedList style={{marginTop: "30px"}}>
     ...
     <ListItem>High-dimensional <strong>naturalistic behaviors,</strong> or complex, <strong>multi-stage</strong> shaped behaviors</ListItem>
...
# flexibility using parallaxin as an example

- python as glue, easy to extend high-level classes implemented with low-level libraries. speed and flexibility!
- extensible modules make it easy to r.y.o and wrap external tools in transforms, hardware, etc.
- networking makes it easy to do complicated things!

# reproducibility

- smart about abstraction, splitting task params and metadata from implementation
